...
Sentry.init({
  ...
  release: `globalmonitor@${__APP_VERSION__}`,
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('globalmonitor-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('globalmonitor-beta-mode', 'true');
    else localStorage.removeItem('globalmonitor-beta-mode');
    location.reload();
  },
});
