...
const BROWSER_ORIGIN_PATTERNS = [
  /^https:\/\/(.*\.)?globalmonitor\.app$/,
  /^https:\/\/globalmonitor-[a-z0-9-]+\.vercel\.app$/,
...
export function validateApiKey(req) {
  const key = req.headers.get('X-GlobalMonitor-Key');
  ...
  if (isDesktopOrigin(origin)) {
    ...
    const validKeys = (process.env.GLOBALMONITOR_VALID_KEYS || '').split(',').filter(Boolean);
    if (!validKeys.includes(key)) return { valid: false, required: true, error: 'Invalid API key' };
    return { valid: true, required: true };
  }

  // Trusted browser origin (globalmonitor.app, Vercel previews, localhost dev) — no key needed
