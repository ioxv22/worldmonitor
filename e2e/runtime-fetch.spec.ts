...
test.describe('desktop runtime routing guardrails', () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        webHost: runtime.detectDesktopRuntime({
          ...
          userAgent: 'Mozilla/5.0',
          locationProtocol: 'https:',
          locationHost: 'globalmonitor.app',
          locationOrigin: 'https://globalmonitor.app',
        }),
      };
    });
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('globalmonitor.app/api/fred-data')) {
        ...
        if (url.includes('globalmonitor.app/api/stablecoin-markets')) {
      ...
      // Set a valid GM API key so cloud fallback is allowed
      await runtimeConfig.setSecretValue('GLOBALMONITOR_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, 'gm_test_key_1234567890abcdef');
      ...
      try {
        ...
        await runtimeConfig.setSecretValue('GLOBALMONITOR_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, '');
      }
    });
    ...
    expect(result.calls.some((url) => url.includes('globalmonitor.app/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('127.0.0.1:46123/api/stablecoin-markets'))).toBe(true);
    expect(result.calls.some((url) => url.includes('globalmonitor.app/api/stablecoin-markets'))).toBe(true);
  });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('globalmonitor.app/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('globalmonitor.app/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
    ...
    expect(result.calls.some((url) => url.includes('globalmonitor.app/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('globalmonitor.app/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/ioxv22/worldmonitor/releases/latest';
    ...
    expect(result.macArm).toBe('https://globalmonitor.app/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://globalmonitor.app/api/download?platform=windows-exe&variant=full');
    expect(result.linuxFallback).toBe('https://github.com/ioxv22/worldmonitor/releases/latest');
  });
  ...
  test('cloud fallback blocked without WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('globalmonitor.app'));
  ...
  test('cloud fallback allowed with valid WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('globalmonitor.app') && init?.headers) {
        ...
        if (url.includes('globalmonitor.app/api/market/v1/test')) {
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('globalmonitor.app')).length,
