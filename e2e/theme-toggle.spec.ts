...
test.describe('theme toggle (happy variant)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      if (!sessionStorage.getItem('__test_init_done')) {
        localStorage.removeItem('globalmonitor-theme');
        localStorage.removeItem('globalmonitor-variant');
        localStorage.setItem('globalmonitor-variant', 'happy');
  ...
  test('dark mode persists across page reload', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('globalmonitor-theme'));
  ...
  test('no FOUC: data-theme is set before main CSS loads', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('globalmonitor-theme', 'dark');
      localStorage.setItem('globalmonitor-variant', 'happy');
    });
